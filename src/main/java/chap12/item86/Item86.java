...
item85에서 말했듯이 객체 정보를 byte stream 으로 넘기므로
모든 필드, 메서드에 접근 가능하고 원격 코드까지 수행할 수 있게 된다
이 부분에서 심각한 보안 취약점으로 작용하고 최악의 경우 native code 까지 원격으로 수행된다
Serializable 구현 시 release 이후 수정이 어렵고
모든 부분을 까서 보여줬으므로 이 부분까지 API 의 일부분인 것이다
즉, implements Serializable 때리는 순간 내부 구현이 공개된다고 생각해야 한다, 그 때문에 캡슐화가 깨지는 것
...
상속용으로 클래스를 작성하는 경우에는 Serializable 구현 시 상속 받은 클래스 모두에서 추가 작업이 필요하다
 */